...
class SimpleList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView(
     ...
     // children: [ // Первый вариант отображения SimpleList
      // MyListItem(number: 1),
      // MyListItem(number: 2),
      // MyListItem(number: 3),
      // MyListItem(number: 4),
...
class SimpleListBuilder extends StatelessWidget {
 ...
 //var list = List<int>.generate(50,(i)=>i+1); // необязательный парметр

  @override
  Widget build(BuildContext context) {
    return ListView.builder(
        padding: const EdgeInsets.all(8),
        itemCount:30, // устанавливать ограничитель ListView
        itemBuilder: (BuildContext context, int index) {
        return MyListItem(number: index+1,); // +1 чтобы начиналось с 1, а не с 0
        }
    );
  }
}
